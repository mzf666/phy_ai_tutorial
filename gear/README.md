...
| module | 关键技术 | 状态 |
|---|---|---|
| [`egoscale/action`](egoscale/action/README.md) | 人类动作表示: 相机位姿 × 21 手部关键点 → 世界系腕部位姿; chunk 内相对腕部 SE(3) 运动; URDF 正运动学 + 22 关节非线性规划的手部重定向 (关节限位、warm start、一阶指数滤波); 三种动作空间 (wrist-only / fingertip / full retargeted joints) 的并排实现与消融 | 未开始 |
