...
**设计理念 (本仓库解读)**: 这就是 Transformer 原始论文的正弦位置编码, 把整数位置换成 [0, 1] 内的连续标量. 网络需要知道当前噪声水平, 但一个标量直接拼进 1024 维激活里几乎没有信号, 后面一个 Linear 一乘就淹没, 而且 MLP 对标量的细微差别不敏感 (Fourier features 讲的谱偏置). 把 τ 展开成一组不同周期的 sin / cos 后: 慢通道 (周期 4) 在 [0, 1] 上只走 1/4 周期, 单调, 告诉网络 "大致在哪"; 快通道 (周期 0.004) 转 250 圈, 能分辨 Δτ ≈ 0.001, 告诉网络 "精确在哪". sin / cos 成对让任意 τ 都能唯一解码, 且 τ 的平移是嵌入的线性变换 (与 RoPE 同一性质). 量程的选择: 上游注释写 "sensitivity in the range [0, 1]"; 上界 4 保证最慢通道在 [0, 1] 内不绕圈; 下界 0.004 覆盖训练时最小的 τ = 0.001 与推理步长 0.1. 对比: Vaswani 的位置是整数, 周期 2π 到 10000·2π; DDPM 用同一函数嵌入 0..1000 的整数步; flow matching 把 t 连续化到 [0, 1], 周期随之缩放, 没有新东西.

**timestep 是什么**: flow matching 的 "去噪进度" τ, 每个样本一个标量, shape [B]. 与机器人时间步 t 无关: t 是控制周期序号 (第几个 20 ms), τ 是当前 action chunk 被噪声污染的程度. 取值: 训练时每个样本独立采 τ ~ Beta(1.5, 1) · 0.999 + 0.001, 在 [0.001, 1.0] 内, 偏向噪声大的一侧 (`pi0.py` L197, 原因 → `../flow_matching`); 推理时是确定性网格 1.0, 0.9, …, 0.1 共 10 个点, 每步 dt = −0.1 (L228, L278). 符号约定: openpi 里 τ = 1 是纯噪声, τ = 0 是干净动作, 与论文正文相反, 作者在 L226-L227 的注释里说明了. 本仓库按代码. τ 只在入口用一次, 广播到 50 个 action token 后与 `action_in_proj` 的输出拼接过 MLP, 18 层里再无 τ 的注入.
