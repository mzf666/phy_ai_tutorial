...
顺序固定, **TL;DR 在最前, 然后 I/O 先行**:

0. **TL;DR**: README 的第一段, 放在首图之前, 用三到五句话说清: (a) 动机: 这个技术要解决什么问题, 问题出现在训练侧还是推理侧, 由谁提出 (论文章节); (b) 做法: 一句话说清机制; (c) 收益与代价: 论文披露的数字 (快多少, 慢多少, 换来了什么), 逐值引用. 紧接着一张 **流程图** (`figs/pipeline.png`, 脚本生成): 这个技术从输入到输出的每一步, 每步标 shape 与一个具体例子的数值, 有逆过程的 (分词 / 归一化 / 加噪) 把逆过程画成镜像的一排. 然后一句话说明每张图的论点: 读者看图时应该看出什么. 不写背景铺垫, 不写本 module 的目录.
...
6. **完成的定义**: `test_parity.py` 在 CPU 上通过; `tiny` 配置的 train / infer 跑通; README 的 TL;DR 与九节齐全; 无 AI 署名.
